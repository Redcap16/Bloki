...
LRESULT CALLBACK Window::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CLOSE:
		...
		return 0;
	case WM_KEYDOWN:
		window.m_Keyboard.keyPressed(wParam);
		for (auto listener : window.m_KeyboardListeners)
			listener->OnKeyboardEvent({ KeyboardListener::KeyboardEvent::KeyPressed, wParam });

		return 0;
	case WM_KEYUP:
		window.m_Keyboard.keyReleased(wParam);
		for (auto listener : window.m_KeyboardListeners)
			listener->OnKeyboardEvent({ KeyboardListener::KeyboardEvent::KeyReleased, wParam });

		return 0;
	case WM_SIZE:
		window.m_Size = { LOWORD(lParam), HIWORD(lParam) };

		for (auto listener : window.m_WindowListeners)
			listener->OnWindowResize(window.m_Size);

		return 0;
	case WM_MOUSEMOVE:
	{
		const glm::ivec2 mousePos = glm::ivec2(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		window.m_Mouse.mouseMove(mousePos);

		for (auto listener : window.m_MouseListeners)
			listener->OnMouseMove(mousePos);
		break;
	}
	case WM_LBUTTONDOWN:
		window.m_Mouse.buttonPressed(Mouse::Button::Left);
		for (auto listener : window.m_MouseListeners)
			listener->OnMouseButtonEvent({MouseListener::MouseButtonEvent::Pressed, Mouse::Button::Left});
		break;
	case WM_LBUTTONUP:
		window.m_Mouse.buttonReleased(Mouse::Button::Left);
		for (auto listener : window.m_MouseListeners)
			listener->OnMouseButtonEvent({ MouseListener::MouseButtonEvent::Released, Mouse::Button::Left });
		break;
	case WM_RBUTTONDOWN:
		window.m_Mouse.buttonPressed(Mouse::Button::Right);
		for (auto listener : window.m_MouseListeners)
			listener->OnMouseButtonEvent({ MouseListener::MouseButtonEvent::Pressed, Mouse::Button::Right });
		break;
	case WM_RBUTTONUP:
		window.m_Mouse.buttonReleased(Mouse::Button::Right);
		for (auto listener : window.m_MouseListeners)
			listener->OnMouseButtonEvent({ MouseListener::MouseButtonEvent::Released, Mouse::Button::Right });
		break;
	}
